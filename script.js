...
function draw() {
    ...
    for (let i = 0; i < canvas.width + 1; i++) {
        let y = canvas.height / 3 + Math.sin(i / 50) * (40 * Math.sin(delta * 0.03));
    ...
    ctx.fill(orangePath);
    //white
    let white = ctx.createLinearGradient(canvas.width, canvas.height / 2, 0, canvas.height);
